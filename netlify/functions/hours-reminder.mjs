...
// fnsrc/hours-reminder-core.mjs
var WORDS = { title: "\u23F1 Crew hours", body: "Time to look at the crew's hours for the week. Open Boiler Room, then Crew portal, then Crew hours." };
...
function due(settings, state, now) {
  const s = settings || {};
  ...
  if (s.every === 2) {
    ...
    const lastSat = Date.parse(p.ymd + "T12:00:00Z") - (p.dow + 1) % 7 * 864e5, weeks = Math.round((lastSat - anchor) / (7 * 864e5));
    if (weeks % 2 !== 0) return { send: false, why: "off-week", ...p };
  }
  return { send: true, why: "due", ...p };
}

// fnsrc/hours-reminder.mjs
var SET = "/Clore DayLog/App Data/hours-reminder.json";
var STATE = "/Clore DayLog/App Data/hours-reminder-state.json";
var hours_reminder_default = async () => {
  ...
  const t = await dbxToken();
  const dl = async (path) => {
    const r = await fetch("https://content.dropboxapi.com/2/files/download", { method: "POST", headers: { Authorization: "Bearer " + t, "Dropbox-API-Arg": JSON.stringify({ path }) } });
    return r.ok ? r.text() : null;
  };
  const read = async (path) => {
    try {
      return JSON.parse(await dl(path) || "null");
    } catch (e) {
      return null;
    }
  };
  const d = due(await read(SET), await read(STATE), /* @__PURE__ */ new Date());
  let sent = 0;
  if (d.send) {
    ...
    await fetch("https://content.dropboxapi.com/2/files/upload", {
      ...
      body: JSON.stringify({ last: d.ymd, sent, at: (/* @__PURE__ */ new Date()).toISOString() })
    });
  }
  const out = { ok: true, sent, why: d.why };
  console.log(JSON.stringify(out));
  return Response.json(out);
};
export {
  hours_reminder_default as default
};
