...
var require_src2 = __commonJS({
  "node_modules/web-push/src/index.js"(exports, module) {
    ...
    module.exports = {
      ...
      setGCMAPIKey: webPush3.setGCMAPIKey,
      setVapidDetails: webPush3.setVapidDetails,
      generateRequestDetails: webPush3.generateRequestDetails,
      sendNotification: webPush3.sendNotification.bind(webPush3)
    };
  }
});

// ../Boiler-Room/fnsrc/client-push.mjs
var import_web_push2 = __toESM(require_src2(), 1);

// ../Boiler-Room/fnsrc/pushlib.mjs
...
// ../Boiler-Room/fnsrc/client-push-core.mjs
var BASE = "/Clore DayLog/App Data/Client Portal";
var KINDS = ["journal", "budget"];
var WORDS = {
  journal: { title: "\u{1F4D6} Project update", body: "A new journal entry was posted on your project page." },
  budget: { title: "\u{1F4B0} Project update", body: "The budget on your project page was updated." }
};
var WINDOW = { from: 9, to: 17 };
var BURST_MIN = 30;
var clean = (s) => String(s || "").replace(/[^a-z0-9-]/gi, "").slice(0, 40);
function alaskaHour(d) {
...
var inWindow = (d) => {
  const h = alaskaHour(d);
  return h >= WINDOW.from && h < WINDOW.to;
};
function payloadFor(kind, c) {
  const w = WORDS[kind];
  return JSON.stringify({ title: w.title, body: w.body, tag: "cp-" + kind, url: "/c/?c=" + clean(c) });
}
function subOk(s) {
  return !!(s && typeof s === "object" && typeof s.endpoint === "string" && /^https:\/\//.test(s.endpoint) && s.endpoint.length < 600 && s.keys && typeof s.keys.p256dh === "string" && typeof s.keys.auth === "string");
}
async function readJson(deps, path, fallback) {
  try {
    const t = await deps.dl(path);
    const j = t == null ? null : JSON.parse(t);
    return j == null ? fallback : j;
  } catch (e) {
    return fallback;
  }
}
async function sendKind(deps, c, kind) {
...
async function ping(deps, body) {
  ...
  if (!inWindow(now)) {
    ...
    if (!arr.some((x) => x && x.c === c && x.kind === kind)) {
      arr.push({ c, kind, ts: now.toISOString() });
      await deps.up(qPath, JSON.stringify(arr, null, 1));
    }
...
async function drain(deps) {
  ...
  for (const x of arr) {
    if (!await burstOk(deps, clean(x.c), x.kind, now)) {
      skipped++;
      continue;
    }
...
// ../Boiler-Room/fnsrc/client-push.mjs
var hsafe = (o) => JSON.stringify(o).split("").map((ch) => ch.charCodeAt(0) > 126 ? "\\u" + ch.charCodeAt(0).toString(16).padStart(4, "0") : ch).join("");
  ...
  return {
    ...
    send: async (sub, payload) => {
      try {
        ...
        return e && (e.statusCode === 404 || e.statusCode === 410) ? "dead" : false;
      }
    }
  };
}
var client_push_default = async (req) => {
...
export {
  client_push_default as default,
  mkDeps
};
