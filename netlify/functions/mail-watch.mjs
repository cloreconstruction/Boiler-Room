  ...
  "../boiler-room-tools/node_modules/bn.js/lib/bn.js"(exports, module) {
    (function(module2, exports2) {
      ...
      BN.prototype.divRound = function divRound(num) {
        ...
        var up2 = new BN(1);
        up2.negative = this.negative ^ num.negative;
        return dm.div.iadd(up2);
      };
...
function mailClean(body) {
  ...
  t = t.replace(/^On .{0,120} wrote:[\s\S]*$/m, "");
...
function mailVerdictParse(text, by) {
  try {
    ...
    return { bucket: "maybe", why: "other", gist: "", by: "none" };
  }
}
...
function pushText(n, why) {
  const title = "\u{1F4E7} Email needs a look";
  const body = n > 1 ? `${Math.min(Math.max(n, 2), 20)} emails look important. Open Boiler Room to sort them.` : MAIL_PUSH[why] || MAIL_PUSH.other;
  return { title, body };
}
function pushSafe(t, b) {
  return !/[$@]/.test(t + b) && !/\d{3,}/.test(t + b);
}
function quietNow(now, quiet) {
...
var api = (tok, path, arg) => fetch("https://api.dropboxapi.com/2/" + path, {
  method: "POST",
...
async function run({ now = /* @__PURE__ */ new Date(), env = process.env, log = () => {
} } = {}) {
  ...
  try {
    ...
    return { ok: false, why: "bad rules" };
  }
  if (!rules || rules.sort === false) {
    log("sorting off \u2014 idle");
    return { ok: false, why: "sort off" };
  }
  let judged = {};
  const jTxt = await dl(tok, JUDGED);
  if (jTxt) {
    try {
      const p = JSON.parse(jTxt);
      if (p && typeof p === "object") judged = p;
    } catch (e) {
    }
  }
  ...
  for (const f of todo) {
    ...
    if (!/^SUBJECT:/im.test(txt)) {
      judged[f.name] = { skip: "text", at: now.toISOString() };
      continue;
    }
    const m = mailParse(txt);
    if (!m.addr && !m.subj && !m.body.trim()) {
      judged[f.name] = { skip: "empty", at: now.toISOString() };
      continue;
    }
    if (m.addr && mailListed(rules.no || [], m.addr)) {
      judged[f.name] = { skip: "never", at: now.toISOString() };
      continue;
    }
    ...
    if (persName || m.addr && mailListed(rules.personal || [], m.addr)) {
      judged[f.name] = { skip: "personal", at: now.toISOString() };
      continue;
    }
    const v = await judge(key, m);
    if (!v) continue;
    let bucket = v.bucket, by = "cloud";
    if (m.addr && mailListed(rules.loud || [], m.addr)) {
      bucket = "important";
      by = "loud";
    } else if (m.addr && mailListed(rules.hush || [], m.addr) && bucket === "important") {
      bucket = "maybe";
      by = "hush";
    }
  ...
  if (ring.length) {
    if (quietNow(now, rules.quiet)) {
      ...
      if (pushSafe(title, body)) {
        try {
          pushed = await sendToAll(title, body, "mail-important");
        } catch (e) {
          pushed = { error: true };
        }
      }
      ring.forEach((r) => {
        r.push = "sent";
        r.pushedAt = now.toISOString();
      });
    }
  }
  const cut = +now - KEEP_DAYS * 864e5;
