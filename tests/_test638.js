...
(async () => {
  ...
  ok('a crew phone never sees it — the render still gates on CREW_NAME (it is Eric\'s decision, not theirs)', await page.evaluate(() =>
    /if \(CREW_NAME\)/.test(mailAskHtml.toString()) && CREW_NAME === ''));   // v6.41 — the card gates on CREW_NAME first thing; v6.76 — the words (and the gate) moved into mailAskHtml, which the summaries page reads too
