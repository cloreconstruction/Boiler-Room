...
(async () => {
  ...
  // 🧙 v7.16 — a fix is no longer written onto EVERY record the answer came from (56 of his entries carried fixes that were not
  // theirs); only onto a record the line names by number, or the answer's one record. These lines name none, from two records.
  ok('✓ SAVE puts every kept fix where a fix goes — the kept answer with its line, the list the Wizard reads (v7.16: not onto records the line does not name) — WITHOUT asking again, and the answer stays up', sv.asked === 0 && sv.open && sv.fixes.length === 2 && sv.fixes.some(f => f === '- hung 40 sheets of siding at Pine Cabin => it was 54 sheets') && sv.fixes.some(f => /pony wall at Oak House => it was the rim joist/.test(f)) && sv.kept.length === 2 && sv.ai.every(a => !/ERIC CORRECTED/.test(a)) && /it was 54 sheets/.test(sv.ctx) && /rim joist/.test(sv.ctx), JSON.stringify(sv));
  ...
  ok('the ✕ on the card, and a DIFFERENT answer coming up, both save what was kept first — onto the answer it belongs to, not the new one', await page.evaluate(() => {
    ...
    const b = (rowB.fix || []).some(f => f.t === 'fix on answer B') && !(rowC.fix || []).length && prefs.wizFixes.some(f => f.t === 'fix on answer B' && f.q === 'summarize this week') && !/fix on answer B/.test(entries.find(e => e.id === s3.id).ai || '') && _lastAnswer.q === 'what about monday' && _wizPend.length === 0 && !document.querySelector('#wizFullBody .wl-fix.pend');
